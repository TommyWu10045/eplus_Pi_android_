...

{
    ...
    public void myToastString(int resId) {
        MyUtils.toastString(getContext(), getResources().getString(resId));
        // UIUtils.showToastSafe(resId);

    }

    public void myToastString(String mess) {
        MyUtils.toastString(getContext(), mess);
    ...
    public boolean noHttp(int what,
                          ...
                          boolean canCancel,
                          boolean isLoading,
                          boolean cancelSign //如果先前的請求未完成，取消先前的請求
    ) {
        try {
            // 初始化请求队列，传入的参数是请求并发值。
            if (mQueue == null) {
            ...
            if (request != null) {
                ...
                mQueue.add(what, request, new HttpResponseListener<>(getActivity(), request, httpListener, canCancel, isLoading));
                return true;
            } else {
                return false;
            }
    ...
    private HttpListener<String> httpListener = new HttpListener<String>() {
        ...
        public void onFailed(int what, Response<String> response) {
            MyUtils.toastString(getContext(), response.getException().getMessage());
        }
    };
...
//      request.add("image1", fileBinary1);// 添加2个文件
    ...
    public void compressPhoto(Context context, String fileName, final int requestCode) {
        ...
        String getPath = WebAndLocalPathRule.getLocaLPath(getContext(), WebAndLocalPathRule.FOLDER_UPLOAD_TEMP);
